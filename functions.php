...
function class_user($name) {
	...
	if ($classy = mysqli_fetch_row($res)) {
		...
		switch ($classy[0]) {
			...
			case 1: // User
				...
				break;
			default:
				$col = ''; // Default if no match
		}

		// Prepare icons based on conditions
		$star = ($classy[1] > 0) ? "<img src='{$site_config['SITEURL']}/images/star.png' alt='donated' border='0'>" : '';
		$warn = ($classy[2] == "yes") ? "<img src='{$site_config['SITEURL']}/images/warned.png' alt='warned' border='0'>" : '';
		$disabled = ($classy[3] == "no") ? "<img src='{$site_config['SITEURL']}/images/disabled.png' alt='disabled' border='0'>" : '';

		// Return the formatted HTML
		return "<font color='{$col}'>{$name}{$star}{$warn}{$disabled}</font>";
	} else {
...
function userlogin() {
	...
	if (
    !isset($_COOKIE["pass"], $_COOKIE["uid"]) ||
    !is_string($_COOKIE["pass"]) ||
    strlen($_COOKIE["pass"]) !== 40 ||
    !ctype_xdigit($_COOKIE["pass"]) ||
...
function torrent_scrape_url($scrape, $hash) {
	...
	$ret = array();
	if ($fp) {
		$stats = BDecode($fp); // Ensure BDecode() returns the expected array
		$binhash = pack("H*", $hash);
		$binhash = addslashes($binhash);

		// Check if the keys exist before accessing them
		$seeds = isset($stats['files'][$binhash]['complete']) ? $stats['files'][$binhash]['complete'] : 0;
		$peers = isset($stats['files'][$binhash]['incomplete']) ? $stats['files'][$binhash]['incomplete'] : 0;
		$downloaded = isset($stats['files'][$binhash]['downloaded']) ? $stats['files'][$binhash]['downloaded'] : 0;
...
function torrenttable($res) {
	...
	foreach ($cols as $col) {
		switch ($col) {
			case 'category':
				echo "<th align='center'>".T_("TYPE")."</th>";
			break;
			case 'name':
				echo "<th align='center'>".T_("NAME")."</th>";
			break;
			case 'dl':
				echo "<th align='center'>".T_("DL")."</th>";
			break;
			case 'uploader':
				echo "<th align='center'>".T_("UPLOADER")."</th>";
			break;
			case 'comments':
				echo "<th align='center'>".T_("COMM")."</th>";
			break;
			case 'nfo':
				echo "<th align='center'>".T_("NFO")."</th>";
			break;
			case 'size':
				echo "<th align='center'>".T_("SIZE")."</th>";
			break;
			case 'completed':
				echo "<th align='center'>".T_("C")."</th>";
			break;
			case 'seeders':
				echo "<th align='center'>".T_("S")."</th>";
			break;
			case 'leechers':
				echo "<th align='center'>".T_("L")."</th>";
			break;
			case 'health':
				echo "<th align='center'>".T_("HEALTH")."</th>";
			break;
			case 'external':
				if ($site_config["ALLOWEXTERNAL"])
					echo "<th align='center'>".T_("L/E")."</th>";
			break;
			case 'added':
				echo "<th align='center'>".T_("ADDED")."</th>";
			break;
			case 'speed':
				echo "<th align='center'>".T_("SPEED")."</th>";
			break;
			case 'wait':
				if ($wait)
					echo "<th align='center'>".T_("WAIT")."</th>";
			break;
			case 'rating':
				echo "<th align='center'>".T_("RATINGS")."</th>";
			break;
		}
	}
	$wait = $wait ?? false; // Default to false if $wait is not set
	...
	foreach ($cols as $col) {
		switch ($col) {
			...
			break;
			case 'comments':
				print("<td class='ttable_col$x' align='center'><button class='Tsiz'><i class='fa-regular fa-message' style='color: Orange; float: left;'></i> <a href='comments.php?type=torrent&amp;id=$id'>" . number_format($row["comments"]) . "</a></button></td>\n");
			break;
			case 'nfo':
				if ($row["nfo"] == "yes")
			...
			break;
			case 'size':
				print("<td class='ttable_col$x' align='center'><button class='Tsiz'><i class='fas fa-database' style='color: Orange; margin-right: 5px;'></i> ".mksize($row["size"])." </button></td>\n");
			break;
			case 'completed':
			...
			case 'seeders':
				if ($row["seeders"] == "0")
				print("<td class='ttable_col$x' align='center'><button class='Tsiz'><i class='fas fa-angle-double-up' style='color: Green; float: left;'></i> ".number_format($row["seeders"])."</button></td>\n");
				else
				print("<td class='ttable_col$x' align='center'><button class='Tsiz'><i class='fas fa-angle-double-up' style='color: Green; float: left;'></i> ".number_format($row["seeders"])."</button></td>\n");
			break;
			case 'leechers':
				if ($row["leechers"] > "0")
				print("<td class='ttable_col$x' align='center'><button class='Tsiz'><i class='fas fa-angle-double-down' style='color: Red; float: left;'></i> " . number_format($row["leechers"]) . " </button></td>\n");
				else
				print("<td class='ttable_col$x' align='center'><button class='Tsiz'><i class='fas fa-angle-double-down' style='color: Red; float: left;'></i> " . number_format($row["leechers"]) . " </button></td>\n");
			break;
			case 'health':
